...
export const LiveMetricsSection: React.FC<LiveMetricsProps> = ({
  ...
  return (
    <section className="relative w-full py-28 sm:py-36 lg:py-48 px-4 sm:px-8 lg:px-12 bg-[#10170e] border-t border-white/[0.06] overflow-hidden">
      {/* Background radial accent */}
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[700px] h-[500px] bg-[#283615]/[0.35] blur-[180px] rounded-full pointer-events-none -z-10" />
      ...
      <div className="max-w-7xl mx-auto space-y-20 sm:space-y-24">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 border-b border-white/[0.08] pb-10">
          <div className="space-y-3">
            ...
            <h2 className="font-display font-black text-3xl sm:text-5xl lg:text-7xl tracking-[-0.04em] uppercase text-[#F4F1E8]">
              VERIFIABLE
              <br />
              <span className="text-[#B8F34A]">PROTOCOL STATE.</span>
            </h2>
          </div>
          <div className="max-w-md space-y-2">
            <div className="flex items-center gap-2 text-xs font-mono text-[#F4F1E8] font-semibold">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
          {metrics.map((metric, i) => (
            <div
              key={i}
              className={`p-8 sm:p-10 rounded-2xl bg-[#151e12] border transition-all duration-300 flex flex-col justify-between space-y-6 ${
                metric.highlight
                  ? "border-[#B8F34A]/40 shadow-[0_0_35px_rgba(184,243,74,0.08)]"
                  : "border-white/[0.08] hover:border-white/[0.18]"
              }`}
            >
              <div className="font-mono text-[11px] tracking-[0.2em] text-[#A0AA98] uppercase">
                {metric.label}
              </div>

              <div
                className={`font-display font-black text-3xl sm:text-4xl lg:text-5xl tracking-tight uppercase ${
                  metric.highlight ? "text-[#B8F34A]" : "text-[#F4F1E8]"
                }`}
              >
                {metric.value}
              </div>

              <div className="pt-4 border-t border-white/[0.06] text-xs font-sans text-[#A0AA98]">
                {metric.sub}
              </div>
            </div>
          ))}
        ...
        <div className="p-6 sm:p-10 rounded-2xl bg-[#151e12] border border-white/[0.08] space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/[0.06] pb-6">
            <div className="flex items-center gap-3">
              <ShieldCheck className="w-5 h-5 text-[#B8F34A]" />
              <span className="font-display font-bold text-base text-[#F4F1E8] uppercase">
                DECENTRALISED CONTRACT REGISTRY
              </span>
            </div>
            <span className="font-mono text-xs text-[#A0AA98] uppercase tracking-widest">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 font-mono text-xs">
            ...
            <div className="space-y-1.5 p-4 rounded-xl bg-white/[0.02] border border-white/[0.05]">
              <div className="text-[#A0AA98] uppercase tracking-wider text-[10px]">
                STAKING CONTRACT
              </div>
              <div className="flex items-center justify-between gap-2 text-[#F4F1E8]">
                ...
                <button
                  ...
                  className="p-1.5 rounded hover:bg-white/[0.1] text-[#A0AA98] hover:text-[#B8F34A] transition-colors"
                  title="Copy Staking Contract Address"
                >
                  {copiedContract ? (
                    <Check className="w-3.5 h-3.5 text-[#B8F34A]" />
                  ) : (
            ...
            <div className="space-y-1.5 p-4 rounded-xl bg-white/[0.02] border border-white/[0.05]">
              ...
              <div className="flex items-center justify-between gap-2 text-[#F4F1E8]">
                ...
                <span className="text-[10px] text-[#B8F34A] font-semibold">
                  6 DECIMALS
                </span>
              </div>
            </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-2">
            <a
              ...
              className="inline-flex items-center gap-1.5 text-xs font-mono text-[#A0AA98] hover:text-[#B8F34A] transition-colors uppercase tracking-wider"
            >
            ...
            <Link
              href="/stats"
              className="inline-flex items-center gap-1.5 text-xs font-mono text-[#F4F1E8] hover:text-[#B8F34A] transition-colors uppercase tracking-wider font-semibold"
            >
